...
#define	R4(v,w,x,y,z,i)	z+=(w^x^y)+blk(i)+0xCA62C1D6+rol(v,5);w=rol(w,30);
	...
	static void sat_SHA1_Final(SHA1_CTX* context, uint8_t digest[SHA1_DIGEST_SIZE])
	{
		...
		sat_SHA1_Update(context, finalcount, 8); 
...
void Sha1::Update(std::string data)
{
	...
	sat_SHA1_Update(&m_sha1, (unsigned char*)data.c_str(), data.length());
}

std::string Sha1::Final()
{
	unsigned char data[SHA1_DIGEST_SIZE];
	sat_SHA1_Final(&m_sha1, data);
	return std::string((char*)data, SHA1_DIGEST_SIZE);
}
